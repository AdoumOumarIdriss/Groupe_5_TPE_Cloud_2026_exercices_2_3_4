...
# ----------------------------------------------------
# Aucune importation nécessaire, on utilise seulement Python standard

# ----------------------------------------------------
# SECTION 2 : Définition des fonctions
# ----------------------------------------------------

def verifier_annee_bissextile(annee):
    """
    Vérifie si une année est bissextile selon les règles du calendrier grégorien
    Règles : Une année est bissextile si :
    1. Elle est divisible par 4
    2. Mais pas par 100 (sauf si divisible par 400)
    """
    # Vérifier si divisible par 400 (années comme 2000)
    if annee % 400 == 0:
        return True
   
    # Vérifier si divisible par 100 mais pas 400 (années comme 1900)
    if annee % 100 == 0:
        return False
   
    # Vérifier si divisible par 4 (années comme 2024)
    if annee % 4 == 0:
        return True
   
    # Si aucune condition n'est remplie, année non bissextile
    return False
...
def obtenir_jours_du_mois(mois, annee):
    ...
    """
    # Liste des jours pour chaque mois (index 0 = janvier, index 11 = décembre)
    jours_par_mois = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31]
   
    # Si c'est février (mois 2) et année bissextile
    if mois == 2 and verifier_annee_bissextile(annee):
        return 29  # Février a 29 jours en année bissextile
   
    # Pour les autres mois, retourner le nombre de jours normal
    # mois-1 car les listes en Python commencent à l'index 0
...
def valider_date(date_str):
    ...
    """
    # Vérifier que la date contient bien des tirets
    if "-" not in date_str:
        return False, "Le format doit être AAAA-MM-JJ avec des tirets"
   
    # Séparer la date en trois parties
    parties = date_str.split("-")
   
    # Vérifier qu'il y a exactement 3 parties
    if len(parties) != 3:
        return False, "Format incorrect. Doit avoir 3 parties séparées par des tirets"
   
    # Extraire année, mois et jour
    annee_str, mois_str, jour_str = parties
   
    # Vérifier que chaque partie ne contient que des chiffres
    if not (annee_str.isdigit() and mois_str.isdigit() and jour_str.isdigit()):
        return False, "L'année, le mois et le jour doivent être des nombres"
    ...
    # Vérifier que l'année est positive
    if annee < 1:
        return False, "L'année doit être un nombre positif"
   
    # Vérifier que le mois est entre 1 et 12
    if mois < 1 or mois > 12:
        return False, f"Le mois doit être entre 1 et 12 (vous avez saisi {mois})"
...
def convertir_date(date_str):
    ...
    Suppose que la date a déjà été validée
    """
    # Séparer la date en parties
    parties = date_str.split("-")
    annee, mois, jour = parties
   # MAHAMAT NOUR ZAKARIA 22B171FS
    # Formater avec des zéros initiaux si nécessaire
    # :02d signifie "formater comme un nombre avec 2 chiffres, ajouter des zéros si besoin"
...
def afficher_exemples():
    ...
    dates_exemples = [
        "2023-12-25",  # Noël 2023 - date valide
        "2023-02-29",  # 29 février 2023 - invalide (non bissextile)
        "2024-02-29",  # 29 février 2024 - valide (bissextile)
...
def afficher_a_propos():
   ...
   #Matricule:22A692FS
    ...
    print("À PROPOS")
    print("="*60)
    print("\n📅 Programme : Convertisseur de dates Python")
    print("\n🎯 Objectif :")
    print("   Convertir les dates du format international")
    print("   AAAA-MM-JJ vers le format français JJ/MM/AAAA")
    print("\n👨‍💻 Auteur :")
    print("   Adoum Oumar Idriss")
    print("   Matricule : 22A692FS")
    print("\n🔧 Fonctionnalités :")
    print("   • Validation complète des dates")
    print("   • Gestion des années bissextiles")
    print("   • Interface utilisateur simple")
    print("   • Messages d'erreur clairs")
    print("\n💻 Technologies :")
    print("   • Python 3 (sans bibliothèques externes)")
    print("="*60)
...
def traiter_conversion():
    """
    Demande une date à l'utilisateur et la convertit
    """
    print("\n" + "="*60)
...
def main():
    """
    Fonction principale qui contrôle le déroulement du programme
    """
    ...
    while True:
        ...
        if choix == "1":
            ...
            # Choix invalide
            print("\n⚠️  Choix invalide. Veuillez saisir 1, 2, 3 ou 4.")
            input("Appuyez sur Entrée pour continuer...")

# ----------------------------------------------------
# SECTION 4 : Point d'entrée du programme
# ----------------------------------------------------
